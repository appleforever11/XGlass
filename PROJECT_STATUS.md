...
The `v1.2.2` workflow moved the same Swift 6.3.3 IRGen crash to remaining method-reference setters in `XGlassSecondarySettingsPages.swift`. All settings, command, and toolbar bindings now use explicit setter closures. The corrected patch is prepared as `1.2.3` without force-moving any failed tag; its tag-triggered run must verify the full test, signing, notarization, and Sparkle publication path.

The `v1.2.3` workflow compiled all 57 tests but two WebKit stress fixtures failed only under SwiftPM's parallel XCTest workers: one isolated view reported a WebKit deinitialization transition and another missed a document-start telemetry script under runner contention. The complete suite passes serially, so the release test gate now runs one XCTest process for deterministic WebKit fixture isolation. No archive or GitHub release was published from `v1.2.3`.

The `v1.2.4` workflow (`34771127065`) passed all 57 serial WebKit tests, built and signed the arm64 archives, notarized the ZIP and DMG, generated a signed Sparkle appcast, and published the GitHub release. The matching annotated tag and `origin/main` are verified at the release commit; the local `dist/XGlass.app` is rebuilt at version `1.2.4` / build `102004` and remains running after signed-bundle verification.
